'''Includes `sf.Dataset` class and its associated functions.
...
into torch DataLoader and tensorflow Dataset objects. A more in-depth look
at the Dataset class is given in slideflow.dataset.__init__.py
'''
...
from slideflow.dataset import dataset_utils
from slideflow.dataset.dataset_utils import (split_patients,
...
class Dataset:
                ...
                if buffer:
                    ...
                    while not task_finished:
                        ...
                        if path is None:
                            q.task_done()
                            break
                        dataset_utils._tile_extractor(path, **extraction_kwargs)
                    ...
                    for slide in slide_list:
                        wsi = dataset_utils._prepare_slide(
                            slide,
    ...
    def slide_manifest(
        ...
        if not low_memory:
            for count in pool.imap(dataset_utils._count_otsu_tiles, wsi_list):
                counts += [count]
